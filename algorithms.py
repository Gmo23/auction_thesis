...
class FPA_AuctionEnvironment:
    ...
    def run_auction(self, max_rounds=10000, convergence_limit=1000):
        ...
        for _ in range(max_rounds):  #'_' as loop index is not needed 
            ...
            rewards = {bidder: 0 for bidder in self.bidders}  # Initialize rewards for all bidders (default 0)
            rewards[winner] = winner.value - winning_bid  # The winner's reward
...
class EpsilonGreedy:
    ...
    def __init__(self, name, value, a=0.1, b=0.002, num_actions=10, alpha = 0.1, gamma = 0.9): #??? move action grid into AuctionEnvironment
        self.name = name
...
bidder1 = EpsilonGreedy(name="Agent1", value=1, a=0.5, b=0.002, num_actions=19, alpha = 0.1, gamma = 0.9)
bidder2 = EpsilonGreedy(name="Agent2", value=1, a=0.5, b=0.002, num_actions=19, alpha = 0.1, gamma = 0.9)

#Run the simulation
simulation = AuctionSimulation(bidders=[bidder1, bidder2], max_rounds=10000, convergence_limit=1000) # No. rounds of 1000000 is okay time wise
